...
![Version](https://img.shields.io/badge/version-0.12.1-3ef27a?style=flat)
...
- La cámara trasera se abre con `getUserMedia` y un bucle de detección, limitado a
  ~10 fps (no a la frecuencia de refresco de la pantalla — hasta 120Hz en iPhones con
  ProMotion, innecesario para seguir un documento sujeto con la mano y muy pesado para
  mantenerlo sostenido), corre el frame a través de OpenCV.js: gris → `GaussianBlur` →
  `Canny` → `findContours` → `approxPolyDP`, buscando el cuadrilátero convexo más
  grande. El contorno detectado se dibuja en tiempo real sobre el preview.
